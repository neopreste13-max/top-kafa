...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  private superKickSound!: Phaser.Sound.BaseSound
  private headerSound!: Phaser.Sound.BaseSound
  
  // Power-up system
  private hasSpeedBoost = false
  private hasPowerBoost = false
  private hasShield = false
  private speedBoostTimer: Phaser.Time.TimerEvent | null = null
  private powerBoostTimer: Phaser.Time.TimerEvent | null = null
  private shieldTimer: Phaser.Time.TimerEvent | null = null
  private speedMultiplier = 1.0
  private powerMultiplier = 1.0
  
  // Super kick system
  private superKickCharge = 0
  private maxSuperKickCharge = 100
  private superKickChargeRate = 10 // Charge per kick
  private isSuperKickReady = false
  ...
  private setupSounds(): void {
    ...
    this.superKickSound = this.scene.sound.add("super_kick", {
      volume: audioConfig.sfxVolume.value
    })
    this.headerSound = this.scene.sound.add("header_hit", {
      volume: audioConfig.sfxVolume.value
    })
  }
  ...
  public applySpeedBoost(duration: number): void {
    // Clear existing speed boost
    if (this.speedBoostTimer) {
      this.speedBoostTimer.destroy()
    }
    
    this.hasSpeedBoost = true
    this.speedMultiplier = 1.5 // 50% speed boost
    
    // Visual effect
    this.setTint(0x4488ff)
    
    console.log(`⚡ ${this.playerSide} SPEED BOOST ACTIVATED for ${duration}ms`)
  ...
  private removeSpeedBoost(): void {
    ...
    if (!this.hasPowerBoost && !this.hasShield) {
      this.clearTint()
    }
    console.log(`⚡ ${this.playerSide} speed boost ended`)
  }

  public applyPowerBoost(duration: number): void {
    if (this.powerBoostTimer) {
      this.powerBoostTimer.destroy()
    }
    ...
    console.log(`💪 ${this.playerSide} POWER BOOST ACTIVATED for ${duration}ms`)
    
    this.powerBoostTimer = this.scene.time.delayedCall(duration, () => {
      this.removePowerBoost()
    })
  }

  private removePowerBoost(): void {
    this.hasPowerBoost = false
    this.powerMultiplier = 1.0
    if (!this.hasSpeedBoost && !this.hasShield) {
      this.clearTint()
    }
    console.log(`💪 ${this.playerSide} power boost ended`)
  }

  public applyShield(duration: number): void {
    if (this.shieldTimer) {
    ...
    console.log(`🛡️ ${this.playerSide} SHIELD ACTIVATED for ${duration}ms`)
  ...
  private removeShield(): void {
    ...
    if (!this.hasSpeedBoost && !this.hasPowerBoost) {
      this.clearTint()
    }
    console.log(`🛡️ ${this.playerSide} shield ended`)
  }
  ...
  public addSuperKickCharge(amount: number = 10): void {
    this.superKickCharge = Math.min(this.superKickCharge + amount, this.maxSuperKickCharge)
    
    if (this.superKickCharge >= this.maxSuperKickCharge && !this.isSuperKickReady) {
      this.isSuperKickReady = true
      console.log(`🔥 ${this.playerSide} SUPER KICK READY!`)
    }
  }

  public canSuperKick(): boolean {
    return this.isSuperKickReady
  }

  public getSuperKickCharge(): number {
    return this.superKickCharge
  }

  public getMaxSuperKickCharge(): number {
    return this.maxSuperKickCharge
  }

  public useSuperKick(): Phaser.Math.Vector2 {
    if (!this.isSuperKickReady) {
      return this.getKickForce() // Regular kick if not ready
    }
    
    // Reset super kick
    this.superKickCharge = 0
    this.isSuperKickReady = false
    
    // Play super kick sound
    this.superKickSound.play()
    
    // Super kick has massive force
    const kickDirection = this.facingRight ? 1 : -1
    const superForce = ballConfig.runningKickForce.value * 2 * this.powerMultiplier
    
    console.log(`🔥 ${this.playerSide} SUPER KICK UNLEASHED! Force: ${superForce}`)
    
    return new Phaser.Math.Vector2(kickDirection * superForce, -200)
  }

  // Header detection (ball is above player's head)
  public isInHeaderPosition(ballY: number): boolean {
    const headY = this.y - (this.targetBodyHeight * 0.9) // Top of player
    return ballY < headY && ballY > headY - 50 && !this.isOnGround
  }

  public performHeader(): void {
    this.headerSound.play()
    console.log(`🎯 ${this.playerSide} HEADER!`)
  }

  // Clear all power-ups on reset
  public clearAllPowerUps(): void {
    if (this.speedBoostTimer) this.speedBoostTimer.destroy()
    if (this.powerBoostTimer) this.powerBoostTimer.destroy()
